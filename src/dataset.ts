 ...
 *
 * The following code examples are calling `await dataset.each(...)` to
...
export abstract class Dataset<T extends DataElement> {
   ...
   * await a.each(e => e.print());
   * ```
   *
   * Batch a dataset of arrays:
   * ```js
   * const b = tf.data.array([[1], [2], [3], [4], [5], [6], [7], [8]]).batch(4);
   * await b.each(e => e.print());
   * ```
   *
   ...
   */
  /** @doc {heading: 'Data', subheading: 'Classes'} */
  async each(f: (input: T) => void): Promise<void> {
  ...
  async forEach(f: (input: T) => void): Promise<void> {
    tf.deprecationWarn(
        ...
        'Please use dataset.each() instead');
    return this.each(f);
  }
   ...
   *  resolve(x*x);
   * }));
   * await a.each(e => e.then(function(value){
   *  console.log(value);
   * }));
   * ```
   *
 ...
 * await ds3.each(e => console.log(JSON.stringify(e)));
 *
 * // If the goal is to merge the dicts in order to produce elements like
 * // {a: ..., b: ...}, this requires a second step such as:
 * console.log('Merge the objects:');
 * const ds4 = ds3.map(x => {return {a: x[0].a, b: x[1].b}});
 * await ds4.each(e => console.log(e));
 * ```
 *
 ...
 * await c.each(e => console.log(JSON.stringify(e)));
 * ```
 */
