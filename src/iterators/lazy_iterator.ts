 ...
 *
 * `.next()` returns a `promise` for the next data in the stream. When an item
 * can be provided successfully, the return value is `{value:T, done:false}`.
 * Calling next() on a closed stream returns `{value:null, done:true}`.
 * Usage example:
 * ```js
 * const a = tf.data.array([1, 2, 3]);
 * const iter = await a.iterator();
 * console.log(JSON.stringify(await iter.next()));
 * ```
 *
 * `.handleErrors()` handles errors thrown on this data stream using a provided
 * handler function.
 * Parameter: `handler` A function that handles any `Error` thrown during a
 * `next()`call and returns true if the stream should continue (dropping the
 * failed call) or false if the stream should quietly terminate.  If the handler
 * itself throws (or rethrows) an `Error`, that will be propagated.
 * Returns a `LazyIterator` of elements passed through from upstream, possibly
 * filtering or terminating on upstream `next()` calls that throw an `Error`.
 * Usage example:
 * ```js
 * const a = tf.data.array([1, 2, 3]).map(x => {
 *  if (x % 2 === 0) {
 *    throw new Error(`Oh no, an even number: ${x}`);
 *  }
 * });
 * const iter = await a.iterator();
 * const errorHandlingIterator = iter.handleErrors(e => console.log(e.message))
 * await errorHandlingIterator.next();
 * await errorHandlingIterator.next();
 * ```
 *
 * `.filter()` filters this stream according to the provided filter method.
 * Parameter: `predicate` A function mapping a stream element to a boolean or a
 * `Promise` for one.
 ...
 * const a = tf.data.array([1, 2, 3]);
 * const iter = await a.iterator();
 * const filterIter = iter.filter(x => x%2 === 0);
 ...
 * const a = tf.data.array([1, 2, 3]);
 * const iter = await a.iterator();
 * const mappedIter = iter.map(x => x*x);
 ...
 * Returns a `LazyIterator` of transformed elements.
 *
 * `.serialMapAsync()` maps the stream through a 1-to-1 transform, forcing
 * serial execution.
 * Parameter: `transform` A function mapping a stream element to a transformed
 * element.
 * Returns a `LazyIterator` of transformed elements.
 *
 * `.flatmap()` maps the stream element through a 1-to-many transform.
 * Parameter: `transform` A function mapping a stream element to an array of
 * transformed element.
 * Returns a `LazyIterator` of transformed elements.
 * Usage example:
 * ```js
 * const a = tf.data.array([1, 2, 3]);
 * const iter = await a.iterator();
 * const mappedIter = iter.flatmap(x => [x, x*x, x*x*x]);
 * console.log(JSON.stringify(await mappedIter.next()));
 * console.log(JSON.stringify(await mappedIter.next()));
 * console.log(JSON.stringify(await mappedIter.next()));
 * console.log(JSON.stringify(await mappedIter.next()));
 ...
 * ```
 *
 * `.forEach()` apply a function to every element of the stream.
 * Parameter: `f` A function to apply to each stream element.
 *
 * `.serialForEach()` apply a function to every element of the stream, forcing
 * serial execution.
 * Parameter: `f` A function to apply to each stream element.  Should return
 * 'true' to indicate that the stream should continue, or 'false' to cause it to
 * terminate.
 *
 * `.rowMajorBatch()` groups elements into batches, represented as arrays of
 * elements. We can think of the elements of this iterator as 'rows' (even if
 * they are nested structures).  By the same token, consecutive values for a
 * given key within the elements form a 'column'.  This matches the usual sense
 * of 'row' and 'column' when processing tabular data (e.g., parsing a CSV).
 ...
 * Returns a `LazyIterator` of batches of elements, represented as arrays of the
 * original element type.
 * Usage example:
 * ```js
 * const a = tf.data.array([{a: 1}, {a: 2}, {a: 3}, {a: 4}, {a: 5}, {a: 6}]);
 * const iter = await a.iterator();
 * const batchedIterator = iter.rowMajorBatch(3);
 * console.log(JSON.stringify(await batchedIterator.next()));
 * console.log(JSON.stringify(await batchedIterator.next()));
 * console.log(JSON.stringify(await batchedIterator.next()));
 * ```
 *
 * `.columnMajorBatch()` groups elements into batches, represented in
 * column-major form. We can think of the elements of this iterator as 'rows'
 * (even if they are nested structures).  By the same token, consecutive values
 * for a given key within the elements form a 'column'.  This matches the usual
 * sense of 'row' and 'column' when processing tabular data (e.g., parsing a
 * CSV). Thus, "column-major" means that the resulting batch is a (potentially
 ...
 * Parameter: `smallLastBatch` Whether to emit the final batch when it has fewer
 * than batchSize elements. Default true.
 * Parameter: `zipFn` (optional) A function that expects an array of elements at
 * a single node of the object tree, and returns a `DeepMapResult`.  The
 ...
 * const a = tf.data.array([{a: 1}, {a: 2}, {a: 3}, {a: 4}, {a: 5}, {a: 6}]);
 * const iter = await a.iterator();
 * const batchedIterator = iter.columnMajorBatch(3);
 * console.log(JSON.stringify(await batchedIterator.next()));
 * console.log(JSON.stringify(await batchedIterator.next()));
 * console.log(JSON.stringify(await batchedIterator.next()));
 * ```
 *
 * `.concatenate()` concatenates this `LazyIterator` with another.
 * Parameter: `iterator` A `LazyIterator` to be concatenated onto this one.
 * Parameter: `baseErrorHandler` An optional function that can intercept
 * `Error`s raised during a `next()` call on the base stream.  This function can
 * decide whether the error should be propagated, whether the error should be
 * ignored, or whether the base stream should be terminated.
 * Returns a concatenated `LazyIterator`.
 * Usage example:
 * ```js
 * const a = tf.data.array([1, 2, 3]);
 * const b = tf.data.array([4, 5, 6]);
 * const iterA = await a.iterator();
 * const iterB = await b.iterator();
 * const iterC = iterA.concatenate(iterB);
 * console.log(JSON.stringify(await iterC.next()));
 * console.log(JSON.stringify(await iterC.next()));
 * console.log(JSON.stringify(await iterC.next()));
 * console.log(JSON.stringify(await iterC.next()));
 * console.log(JSON.stringify(await iterC.next()));
 ...
 * ```
 *
 * `.take()` limits this stream to return the specified number of items.
 * Parameter: `count` The maximum number of items to provide from the stream. If
 ...
 * const iter = await a.iterator();
 * const takedIter = iter.take(3);
 * console.log(JSON.stringify(await takedIter.next()));
 * console.log(JSON.stringify(await takedIter.next()));
 * console.log(JSON.stringify(await takedIter.next()));
 * console.log(JSON.stringify(await takedIter.next()));
 * ```
 *
 * `.skip()` skips the specified number of items in this stream.
 * Parameter: `count` The number of items to skip.  If a negative or undefined
 ...
 * console.log(JSON.stringify(await skippedIter.next()));
 * console.log(JSON.stringify(await skippedIter.next()));
 * console.log(JSON.stringify(await skippedIter.next()));
 * console.log(JSON.stringify(await skippedIter.next()));
 * ```
 ...
 * Returns a `LazyIterator`.
 *
 * `.shuffle()` randomly shuffles the elements of this stream.
 * Parameter: `bufferSize` An integer specifying the number of elements from
 * this stream from which the new stream will sample.
 * Parameter: `seed` (Optional.) An integer specifying the random seed that
 ...
 * const a = tf.data.array([1, 2, 3, 4, 5, 6]);
 * const iter = await a.iterator();
 * const shuffledIter = iter.shuffle(3)
 * console.log(JSON.stringify(await shuffledIter.next()));
 * console.log(JSON.stringify(await shuffledIter.next()));
 * console.log(JSON.stringify(await shuffledIter.next()));
 * console.log(JSON.stringify(await shuffledIter.next()));
 * console.log(JSON.stringify(await shuffledIter.next()));
